...
const AltoProvider: FC<PropsWithChildren> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (printSpace?.TextBlock) {
      setTextBlocks(
        addMetadata(printSpace.TextBlock, printSpace['@_STYLEREFS'])
      );
    } else {
      setTextBlocks([]);
    }
  ...
  const updateGraphicalElement = useCallback(
    (graphicalElement: any, index: number) => {
      setAlto((old: any) => {
        if (index === -1) {
          return {
            ...old,
  ...
  const updateTextLine = useCallback(
    (textLine: any, textBlockIndex: number, textLineIndex: number) => {
      if (printSpace) {
        const textBlock =
          textBlockIndex === -1
  ...
  const updateString = useCallback(
    (
      ...
      if (printSpace) {
        ...
        } else if (textStringIndex === -1) {
          textBlock.TextLine[textLineIndex].String['@_CONTENT'] = value;
        } else {
          textBlock.TextLine[textLineIndex].String[textStringIndex][
            '@_CONTENT'
          ] = value;
        }
        updateTextBlock(textBlock, textBlockIndex);
