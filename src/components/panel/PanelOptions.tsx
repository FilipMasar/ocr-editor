...
import { Info, Save, Trash, Upload } from "react-feather"
...
const PanelOptions:FC = () => {
	...
	const onStartOver = (e: MouseEvent<HTMLButtonElement>) => {
		...
		if (window.confirm("Are you sure you want to start over? Make sure to save your work first.")) {
			setAlto(undefined)
	...
	return (
		<>
			...
			{saving && (
				<div className="absolute bottom-0 right-0 m-4 bg-lime-400 rounded-lg px-4 py-2 flex gap-2 items-center bg-">
					<Info size={20} />
        Saved!
				</div>
			)}
