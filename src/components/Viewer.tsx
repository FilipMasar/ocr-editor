  ...
  return (
    <div style={{position: "relative", width: "100%", height: "100%", margin: 20}}>
      ...
      {illustrations.map((illustration: any, index: number) =>
        <Illustration key={index} top={illustration["@_VPOS"]} left={illustration["@_HPOS"]} width={illustration["@_WIDTH"]} height={illustration["@_HEIGHT"]} />
      )}

      {graphicalElements.map((graphicalElement: any, index: number) =>
        <GraphicalElement key={index} top={graphicalElement["@_VPOS"]} left={graphicalElement["@_HPOS"]} width={graphicalElement["@_WIDTH"]} height={graphicalElement["@_HEIGHT"]} />
      )}
      ...
      {strings.map((string: any, index: number) =>
        <String key={index} top={string["@_VPOS"]} left={string["@_HPOS"]} width={string["@_WIDTH"]} height={string["@_HEIGHT"]} text={string["@_CONTENT"]} lineVPos={string.lineVPos}/>
      )}
    </div>
  );
};
